...
impl ReactTextElement {
    ...
    fn build_child(&self, window: &mut Window) -> AnyElement {
        ...
        if let Some(lh) = style.line_height {
            ...
            // measured ink-row parity vs headless Chrome is within 0.5px with NO
            // extra nudge. An older gpui split leading from the font metric box and
            // needed a `baseline_correction` shim here — that shim over-shifted all
            // line-height'd text ~1.5-2px down on current gpui (the "tab label sits
            // low" bug) while hiding inside the parity diff's AA floor. Pinned by

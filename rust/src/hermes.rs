//! Embedded Hermes JS engine on a dedicated thread — the single-process replacement for
//! the Bun-process + NDJSON-pipe bridge. The Rust binary owns the macOS main thread for
//! GPUI/Metal; this module runs React on a JS thread and talks to it through the
...
extern "C" fn host_close(_ud: *mut c_void, _arg: *const c_char) {}
...
// re-render for the whole batch, instead of one sync re-render per event on the legacy root).

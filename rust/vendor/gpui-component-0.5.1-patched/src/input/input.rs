...
impl RenderOnce for Input {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            ...
            .input_px(self.size)
            .input_py(self.size)
            // PATCHED (react-native-gpui): single-line vertical centering. A fixed
            // `input_h` fought `size_full` — when the host wraps this Input in a taller
            // field box (e.g. a 40px composer), the 32px wrapper top-aligned inside it and
            // the text/placeholder sat high. When `appearance` is false the Input is
            // embedded in the host's own styled box, so let `size_full` fill that box and
            // make the fixed height a *minimum* — then `.items_center()` truly centers the
            // editor. Standalone (`appearance` true) inputs keep the fixed pill height.
            .map(|this| {
                if self.appearance || self.height.is_some() {
                    this.input_h(self.size)
                } else {
                    // mirror input_h's fixed heights (h_5/h_6/h_8/h_11) as a minimum
                    let min_h = match self.size {
                        Size::Large => px(44.),
                        Size::Small => px(24.),
                        Size::XSmall => px(20.),
                        _ => px(32.),

...
// offscreen visual-conformance harness drives focus. Upstream gates the caret on
// `window.is_window_active()`, but the test harness runs the window non-activating
// (RNGPUI_NO_ACTIVATE / RNGPUI_TEST_MODE), so the caret would never paint and the
// caret pixel gate could never validate width/color/blink. The blink itself is
// unaffected (it still toggles `BlinkCursor::visible()`), so the blink gate still works.
fn cursor_in_inactive_window() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        std::env::var_os("RNGPUI_CARET_IN_INACTIVE_WINDOW").is_some()

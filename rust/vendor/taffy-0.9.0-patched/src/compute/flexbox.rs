                    ...
                    let item_main_size_sum = line
                        ...
                        .map(|item| {
                            ...
                            let flex_contribution = if item.content_flex_fraction > 0.0 {
                                ...
                                // react-native-gpui patch. This must be the SAME scaled shrink
                                // factor the fraction was divided by above — the multiply is the
                                // inverse of that divide, and taffy deliberately skips the spec's
                                // line-max-fraction step, so `flex_basis + contribution` should
                                // come back out as the item's own outer contribution.
                                //
                                // Upstream floors the shrink factor and THEN multiplies by the
                                // basis (`max(1, shrink) * basis`) while the divide floors the
                                // product (`max(1, shrink * basis)`). The two agree whenever
                                // `shrink * basis >= 1`, which is why this survived: the default
                                // web `flex-shrink: 1` always hits that. React Native defaults to
                                // `flexShrink: 0`, so in a gpui/RN tree EVERY item takes the
                                // broken path, and the factor becomes the item's whole basis.
                                // An item with `width: 15, marginLeft: -1, flexShrink: 0` then
                                // contributes -15 to its content-sized parent instead of 14, so
                                // the parent lays out 29px narrower than its children and clips
                                // them. Still present in taffy 0.13.0; see
                                // examples/negative-margin-conformance.tsx.
                                let scaled_shrink_factor = f32_max(1.0, item.flex_shrink * item.inner_flex_basis);
